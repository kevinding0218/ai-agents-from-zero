...
所以，对初学者来说，可以先记住一句话：**没有 Tool，大模型主要是在“说”；有了 Tool，它才开始能“做”。**
...
如果你第一次接触装饰器，不需要把它想得太玄。对本章来说，你只需要先记住：**装饰器的作用，就是在不改函数核心逻辑的前提下，给函数额外加上一层“框架可识别的能力”。**
...
所以它非常适合用在 Tool 参数定义里。对初学者来说，可以先记住一句话：
...
- 从掌握结果看，学完本章后，你至少应该：能清楚区分 **Tool、Tool Calling / Function Calling、Agent** 三层概念；会用 `@tool` 和 `args_schema` 定义一个结构清晰、可被模型理解的工具；能说清楚一次工具调用闭环里谁负责决策、谁负责执行、结果怎么回到消息流。
