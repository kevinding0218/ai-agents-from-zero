...
**学习建议：** 本章开始看基础代码。阅读顺序建议按工程链路来：**配置入口 -> 请求上下文 -> 实时进度 -> 文件工具 -> 模型与提示词**。这样看下来，每个模块都有位置，不会像散落的工具函数。
...
项目对应文件：`deepsearch-agents/app/agent/llm.py`
...
项目对应文件：`deepsearch-agents/app/agent/prompts.py`
