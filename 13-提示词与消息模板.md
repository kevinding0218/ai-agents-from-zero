...
所以，对初学者来说，可以先记住一句话：**Prompt 不只是把一句话写漂亮，而是把模型输入组织清楚。**
...
- **LangChain Prompts / ChatPromptTemplate 参考**：https://reference.langchain.com/python/langchain_core/prompts/
- **LangSmith Prompt Engineering Concepts**：https://docs.langchain.com/langsmith/prompt-engineering-concepts
- **DeepSeek 官方提示词示例库**：https://api-docs.deepseek.com/zh-cn/prompt-library/
...
也就是说，这一章不是零散的 API 介绍，而是在回答一个更大的问题：**当我们真正开发一个 LLM 应用时，应该怎样把“输入”组织成一套可复用、可维护、可扩展的结构？**

---

## 2、调用大模型的入参类型

初学者最容易有一个误解：以为调用聊天模型时，输入永远只能是“一段字符串”。实际上，LangChain 聊天模型为了适配真实对话场景，通常支持多种输入形态。它们表面写法不同，但本质上都在表达一件事：**你希望把哪些内容，以什么角色和什么结构，交给模型。**

### 2.1 入参形态总览

同一次 `invoke`，左侧可以是多种类型的输入，中间由聊天模型处理，右侧典型返回值是 `AIMessage`。这也是为什么你在 [第 11 章](11-Model-I-O与模型接入.md) 会经常看到“正文一般通过 `.content` 读取”。

![聊天模型 invoke：常见入参类型与 AIMessage 输出](images/13/13-2-1-2.png)
...
**建议下一步：** 继续学习 [第 14 章 输出解析器](14-输出解析器.md)，把本章的“输入组织”与下一章的“输出结构化”连起来；再配合 [第 15 章 LCEL 与链式调用](15-LCEL与链式调用.md)，就能形成 LangChain 中最核心的“输入 -> 模型 -> 输出 -> 链式编排”主线。
