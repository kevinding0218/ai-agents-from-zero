...
![从提示词、RAG、微调、续训到智能体的方案诊断关系图](../images/1-3/1-3-1-1-1.png)
...
RAG 的重点不是“修改模型参数”，而是**在运行时给模型补充它当前需要的上下文**。
...
![RAG 的基本流程：先检索外部资料，再把结果与问题一起交给大模型生成答案](../images/1-3/1-3-2-1-1.png)

### 2.2 为什么 RAG 往往是项目里的第一选择

从工程角度看，RAG 之所以常常先于微调和续训，不是因为它“最先进”，而是因为它**最符合大量知识类业务问题的真实成因**。
...
这三类问题，本质上都更像“模型没看到材料”，而不是“模型不会说话”。这时最先应该验证的，往往不是训练模型，而是**能不能把资料喂对**。RAG 正是解决这个问题的。
...
![RAG、微调等方案在“补知识”和“改行为”上的分工对比示意图](../images/1-3/1-3-3-6-1.png)
...
多数企业项目用不到续训。因为它成本高、风险高、迭代慢，很多问题用 `RAG + Prompt` 或 `RAG + 微调` 已经能解决。
...
![典型 LLM Agent 架构：模型、记忆、工具、规划和行动执行协同工作](../images/1-3/1-3-5-1-1.jpg)
...
![Function Calling 的基本流程：模型决定工具和参数，程序负责真正执行](../images/1-3/1-3-5-3-1.jpg)

![Function Calling 在实际接口调用中的工具声明与参数返回示意图](../images/1-3/1-3-5-3-2.jpg)
...
![MCP 像 AI 世界的 USB-C 接口，用统一协议暴露工具、资源和提示模板](../images/1-3/1-3-5-3-3.jpg)
...
![MCP 的调用流程：主机、客户端和服务器之间如何完成能力发现与调用](../images/1-3/1-3-5-3-4.jpg)
...
![工作流更强调固定步骤、可控执行和明确顺序的流程编排方式](../images/1-3/1-3-5-5-1.jpg)
...
**建议下一步：** 建议按你最关心的能力线继续往下走：想把 RAG 落到代码里，就看 [第 19 章 RAG 检索增强生成](19-RAG检索增强生成.md)；想理解工具接入和协议层，就看 [第 20 章 MCP 模型上下文协议](20-MCP模型上下文协议.md)；想理解多步决策与执行，就看 [第 21 章 Agent 智能体](21-Agent智能体.md)。
